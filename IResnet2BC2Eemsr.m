...
nModels = 6;
myNets = [];
save_net_fileT = '~/data/ir_eswarm';
%save_s1net_fileT = '~/data/ir_eswarm1_sv';
save_s2net_fileT = '~/data/ir_eswarm2_sv';
...
t2 = clock();
fprintf('IResnet2 training N images:%d time:%.3f, models %d\n', trainCount, etime(t2, t1), nModels);
...
t2 = clock();
fprintf('IResnet2 Supervisor training N images:%d time:%.3f, models %d\n', nImgsTot, etime(t2, t1), nModels);
...
[testSets, testDataSetFolders] = createBCtestEe(dataFolderTmpl, dataFolderSfx, @readFunctionTrainIN_n);
...
fd = fopen( strcat('predict_ir_6bfmsr',int2str(nModels),'.txt'),'w' );
...
fprintf('IResnet2 test N images:%d, models %d\n', nImgsTot, nModels);
