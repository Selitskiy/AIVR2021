...
% Create imageDataset vector of images in selected makeup folders
[testSets, testDataSetFolders] = createBCtestE(dataFolderTmpl, dataFolderSfx, @readFunctionTrainGN_n);
...
for i=1:nMakeups   
    ...
    parfor s=1:nModels 
        ...
        predictedLabels = classify(myNets(s), testSet); %testSets{i});
        predictedLabelsSwarm{i, s} = predictedLabels;
        predictedScores = predict(myNets(s), testSet); %testSets{i});
